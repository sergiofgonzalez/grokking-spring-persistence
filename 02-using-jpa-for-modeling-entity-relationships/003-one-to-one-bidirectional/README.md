# 003 &mdash; One-to-One: Bidirectional

> One-to-One bidirectional association represented by two distinct tables in the database

## Description

In the example, we define a couple of domain classes `User` and `Email` featuring a *one-to-one* association.
The classes are annotated in a way that a table is used for each of the entities (users and emails) and the relationship is navigable in both directions (i.e. from *User* to *Email* and vice versa).
