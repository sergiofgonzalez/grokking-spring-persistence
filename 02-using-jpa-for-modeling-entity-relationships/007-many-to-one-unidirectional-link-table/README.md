# 007 &mdash; Many-to-One: Unidirectional with a link table

> Many-to-One unidirectional association represented by two distinct tables and a link table in the database

## Description

In the example, we define a couple of domain classes `User` and `Email` featuring a *one-to-many* association navigable from the `Email` to the `User` instance. Therefore, it is seen in the Java code as a *many-to-one*.
