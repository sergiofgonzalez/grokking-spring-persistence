# 010 &mdash; Many-to-Many: Unidirectional with a link table (email owning the relationship)

> Many-to-Many unidirectional association represented by two distinct tables and a link table in the database. An Email is not allowed to be assigned to the same User twice.

## Description

In the example, we define a couple of domain classes `User` and `Email` featuring a *many-to-many* association navigable from the `Email` to the associated `User` instances.
