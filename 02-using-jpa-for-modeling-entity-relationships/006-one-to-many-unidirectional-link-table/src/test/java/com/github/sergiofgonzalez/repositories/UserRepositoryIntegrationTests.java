...
public class UserRepositoryIntegrationTests {
	...
	public void testDeleteUserWithAssociatedEmailAutomaticallyCascadesToLinkTable() {
	...
	public void testCheckUserEmails() {
		...
		assertThat(user2.getEmails()).hasSize(2);
		assertThat(user2.getEmails().stream().map(email -> email.getEmail()).collect(toList())).containsExactlyInAnyOrder("user21@example.com", "user22@example.com");		

		assertThat(user3.getEmails()).hasSize(3);
		assertThat(user3.getEmails().stream().map(email -> email.getEmail()).collect(toList())).containsExactlyInAnyOrder("user31@example.com", "user32@example.com", "user33@example.com");		
	}		
