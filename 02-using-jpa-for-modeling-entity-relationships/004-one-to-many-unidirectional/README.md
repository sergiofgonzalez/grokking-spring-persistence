# 004 &mdash; One-to-Many: Unidirectional

> One-to-Many unidirectional association represented by two distinct tables in the database

## Description

In the example, we define a couple of domain classes `User` and `Email` featuring a *one-to-many* association only navigable from the `Email` to the `User`.
